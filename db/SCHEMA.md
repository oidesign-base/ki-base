...
| `migrations/001_init.sql` | Перша міграція: те саме + запис у `schema_migrations` |
| `seeds/001_reference_data.sql` | Довідники: платформи, їхні показники, перевізники |
| `SCHEMA.md` | Цей опис |

Кожна наступна зміна структури — новий файл `migrations/002_….sql`; `schema.sql` оновлюється разом із ним.
...
- **Статуси** зберігаються англійськими кодами (`ENUM`); українські підписи — в інтерфейсі.
...
**`code_sheets`, `code_pool` — наклейки.** Аркуші друкуються наперед; кожен код — 7 цифр (6 + контрольна), статус `free` / `assigned` / `spoiled`. Екземпляр посилається на код; присвоїти код, якого немає в пулі, база не дасть. При видаленні екземпляра код звільняється (поле стає порожнім).
...
- код наклейки не з 7 цифр або код, якого немає в пулі;
...
- **Etsy:** валюта магазину не задана. **OLX, Facebook:** хто платить комісію — не задано.
